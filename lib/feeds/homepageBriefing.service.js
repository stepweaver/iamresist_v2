...
function briefingTextTokens(text) {
...
function coherenceSignalForEntries(anchor, candidate) {
  ...
  const anchorAge = hoursSincePublished(anchor.intelItem?.publishedAt ?? null);
  const candidateAge = hoursSincePublished(
    candidate.kind === 'newswire' ? candidate.story?.publishedAt ?? null : candidate.intelItem?.publishedAt ?? null,
  );
  if (anchorAge != null && candidateAge != null && Math.abs(anchorAge - candidateAge) > 48) return null;

  const anchorTokens = briefingTextTokens(
    [briefingEntryText(anchor), ...(anchor.intelItem?.promotionReasons ?? [])].filter(Boolean).join('\n'),
  );
  const candidateTokens = briefingTextTokens(briefingEntryText(candidate));
  if (anchorTokens.length === 0 || candidateTokens.length === 0) return null;

  const sharedTokens = overlapCount(anchorTokens, candidateTokens);
  const overlapScore = tokenJaccard(anchorTokens, candidateTokens);
  const sharedEventType =
    candidate.kind === 'intel' &&
    briefingEntryEventType(anchor) &&
    briefingEntryEventType(candidate) &&
    briefingEntryEventType(anchor) === briefingEntryEventType(candidate);

  const sameStory =
    sharedTokens >= Math.max(STORY_COHERENCE_MIN_SHARED_TOKENS + 1, 4) ||
    (sharedTokens >= STORY_COHERENCE_MIN_SHARED_TOKENS && overlapScore >= 0.2) ||
  ...
  return {
    ...
    sharedEventType: Boolean(sharedEventType),
    sharedTokens,
  };
}
