...
import {
  ...
  AGENDA_PULSE_HOMEPAGE_THRESHOLD,
...
import { deriveThemeAttentionSignal } from '@/lib/intel/themeAttentionRanking';
...
function agendaPulseForEntry(entry) {
  if (!entry || entry.kind !== 'intel') return null;
  const pulse = computeAgendaPulseScore(entry.intelItem ?? {});
  if (pulse.score < AGENDA_PULSE_HOMEPAGE_THRESHOLD) return null;
  return pulse;
}
...
function buildBriefingExplain(entry, origin, selectionPhase, homepageScore) {
  ...
  return {
    ...
    agendaPulse:
      entry.kind === 'intel' && entry.agendaPulse
        ? {
            reservedSlot: Boolean(entry.agendaPulseReservedSlot),
            score: entry.agendaPulse.score,
            explanations: entry.agendaPulse.explanations,
            publicConsequenceTags: entry.agendaPulse.publicConsequenceTags,
          }
        : null,
    themeAttention:
      entry.kind === 'intel' && entry.intelItem?.themeAttentionRanking
        ? {
            mode: entry.intelItem.themeAttentionRanking.mode ?? null,
            eligible: Boolean(entry.intelItem.themeAttentionRanking.eligible),
            contribution: entry.intelItem.themeAttentionRanking.contribution ?? 0,
            appliedContribution: entry.intelItem.themeAttentionRanking.appliedContribution ?? 0,
            reasons: Array.isArray(entry.intelItem.themeAttentionRanking.reasons)
              ? entry.intelItem.themeAttentionRanking.reasons
              : [],
            matchedThemeId: entry.intelItem.themeAttentionRanking.matchedThemeId ?? null,
            attentionIsNotCorroboration: true,
          }
        : entry.themeAttentionRanking
          ? {
              mode: entry.themeAttentionRanking.mode ?? null,
              eligible: Boolean(entry.themeAttentionRanking.eligible),
              contribution: entry.themeAttentionRanking.contribution ?? 0,
              appliedContribution: entry.themeAttentionRanking.appliedContribution ?? 0,
              reasons: Array.isArray(entry.themeAttentionRanking.reasons)
                ? entry.themeAttentionRanking.reasons
                : [],
              matchedThemeId: entry.themeAttentionRanking.matchedThemeId ?? null,
              attentionIsNotCorroboration: true,
            }
          : null,
  };
...
export function mergeAndRankBriefingCandidates(candidates) {
  ...
  const scored = prelim.map((c) => {
    ...
    const agendaPulse = agendaPulseForEntry(c);
    return {
      ...c,
      coherenceSupport,
      agendaPulse,
  ...
  const hasAgendaPulse = out.some((entry) => entry.agendaPulse?.score >= AGENDA_PULSE_HOMEPAGE_THRESHOLD);
  if (!hasAgendaPulse && out.length > 0) {
    const heroKey = itemDedupeKey(out[0]);
    const agendaCandidate = deduped
      .filter((entry) => entry.agendaPulse?.score >= AGENDA_PULSE_HOMEPAGE_THRESHOLD)
      .filter((entry) => itemDedupeKey(entry) !== heroKey)
      .filter((entry) => !out.some((o) => sameBriefingEntry(o, entry)))
      .sort((a, b) => {
        if ((b.agendaPulse?.score ?? 0) !== (a.agendaPulse?.score ?? 0)) {
          return (b.agendaPulse?.score ?? 0) - (a.agendaPulse?.score ?? 0);
        }
        return b.homepageBriefingScore - a.homepageBriefingScore;
      })[0];
    ...
    if (agendaCandidate) {
      const selectedEntry = {
        ...agendaCandidate,
        agendaPulseReservedSlot: true,
      };
      const withExplain = {
        ...selectedEntry,
        briefingExplain: {
          ...buildBriefingExplain(selectedEntry, selectedEntry.briefingOrigin ?? 'lane_backstop', 'primary', selectedEntry.homepageBriefingScore),
          agendaPulse: {
            reservedSlot: true,
            score: selectedEntry.agendaPulse.score,
            explanations: selectedEntry.agendaPulse.explanations,
            publicConsequenceTags: selectedEntry.agendaPulse.publicConsequenceTags,
          },
        },
      };
      if (out.length < BRIEFING_TOTAL_SLOTS) {
        out.push(withExplain);
      } else {
        out[out.length - 1] = withExplain;
      }
    }
  }
...
async function newswireCandidatesFromStories(stories) {
  ...
  return pool
    .map((story) => {
      ...
      let raw = bridgeNewswireStoryScore(story, missionScope.scoreDelta ?? 0);
      const themeAttentionRanking = deriveThemeAttentionSignal(
        byId.get(String(story?.id || story?.url || '')) ?? null,
        {
          publishedAt: story?.publishedAt ?? null,
          missionScopeState: missionScope?.scopeState ?? null,
          relevanceScore: 50,
          baseDisplayPriority: raw,
        },
        mode,
      );
      if (themeAttentionRanking.appliedContribution > 0) {
        raw = clampScore(raw + themeAttentionRanking.appliedContribution, 0, 100);
      }
      ...
      return {
        ...
        rawScore: raw,
        weightedScore: weightedBriefingScore({ briefLane, rawScore: raw }),
        themeAttentionRanking: mode === 'off' ? null : themeAttentionRanking,
        story: {
          ...story,
          missionScope,
        },
...
async function buildHomeLiveBriefing() {
  ...
  const nw = await newswireCandidatesFromStories(newswireStories ?? []);
