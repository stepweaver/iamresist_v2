import 'server-only';

import pLimit from 'p-limit';

import { polishFeedCardImageUrl, shouldSkipFeedImageCandidate } from '@/lib/feeds/feedItemImage.js';
import { fetchOgImageFetchCached } from '@/lib/feeds/ogImage.js';
import {
  intelDbConfigured,
  fetchSourceIdsForDeskLane,
  fetchSurfacedSourceItemsForLive,
  fetchDownrankedSourceItemsForLive,
  fetchSuppressedSourceItemsForLive,
  fetchIntelFreshnessForDeskLane,
  saveLiveDeskSnapshot,
  loadLiveDeskSnapshot,
} from '@/lib/intel/db';
import {
  parseDeskDownrankedFetchLimit,
  parseDeskMaxVisibleItems,
  parseDeskSuppressedFetchLimit,
  parseDeskSurfacedFetchLimit,
} from '@/lib/intel/deskLimits';
import { applyDuplicateClusterOverlay, compareDeskItems, provenanceRank } from '@/lib/intel/rank';
import { computeDisplayPriority } from '@/lib/intel/displayPriority';
import { computeTrustWarnings } from '@/lib/intel/trustWarnings';
import { whyItMattersStub } from '@/lib/intel/whyItMatters';
import { computeAccountabilityHighlights } from '@/lib/intel/accountabilitySignals';
import { assessMissionScope } from '@/lib/intel/missionScope';
import { computeCreatorCorroborationBridge } from '@/lib/intel/globalPromotion';
import { capShortWindowCreatorBoost } from '@/lib/intel/themeAttentionRanking';
import { prefetchThemeAttentionByItemId } from '@/lib/intel/themeAttentionPrefetch';
import { assembleStoryClusters } from '@/lib/intel/storyContext';
import {
  applyAnecdotalIndicatorClassCaps,
  applyWatchdogLeadCorroborationRules,
} from '@/lib/intel/watchdogDeskPromotion';
import { partitionDeskRowsForPipeline } from '@/lib/intel/defaultDeskSurface';
import {
  getAgeHours,
  getFreshnessBucket,
  isFreshEnoughForSurface,
  getFreshnessExclusionReason,
} from '@/lib/intel/freshnessPolicy';
import { unstable_cache } from 'next/cache';

function parseOgFallbackEnabled() {
  return String(process.env.INTEL_DESK_OG_FALLBACK || '').trim() === '1';
}

// Keep TTL long enough to avoid repeated rebuilds; ingest-driven tag invalidation remains
// the primary freshness trigger for live desks and homepage briefing desk slices.
const LIVE_INTEL_DESK_CACHE_TTL_SECONDS = 120;
export const PUBLIC_OSINT_PAYLOAD_BUDGET_BYTES = 1_500_000;

const PUBLIC_DESK_STRING_LIMITS = {
  title: 220,
  summary: 320,
  trustExplain: 220,
  trustWarningText: 220,
  whyItMatters: 220,
  explanationMessage: 180,
  accountabilityExplanation: 160,
  creatorReason: 120,
};

function isFederalRegisterCanonical(url) {
  if (!url || typeof url !== 'string') return false;
  try {
    const u = new URL(url);
    return /(^|\.)federalregister\.gov$/i.test(u.hostname);
  } catch {
    return false;
  }
}

/**
 * Desk composition (Milestone 1.8):
 * 1) Fetch surfaced rows only (newest first) — suppressed do not consume this budget.
 * 2) Fetch a small downranked pool second.
 * 3) Merge → deterministic duplicate_cluster overlay → split losers into duplicateItems.
 * 4) Sort remainder provenance-first (compareDeskItems) → cap to INTEL_DESK_MAX_VISIBLE_ITEMS.
 * 5) Suppressed: separate query, disclosure only.
 */

/** PostgREST returns this until `intel` is added under Supabase → Settings → API → Exposed schemas. */
function isIntelSchemaNotExposedError(msg) {
  return /invalid schema:\s*intel/i.test(String(msg));
}

function augmentLiveDeskErrorMessage(msg) {
  if (isIntelSchemaNotExposedError(msg)) {
    return `${msg} — In Supabase: Project Settings → API → Exposed schemas → add "intel". Also run both SQL files in supabase/migrations/ for this repo.`;
  }
  return msg;
}

function parseStaleAfterMinutes() {
  const raw = process.env.INTEL_DESK_STALE_AFTER_MINUTES;
  if (raw == null || String(raw).trim() === '') return 90;
  const n = parseInt(String(raw), 10);
  return Number.isFinite(n) && n > 0 ? n : 90;
}

function ageMinutesFromIso(isoString) {
  if (!isoString) return null;
  const ts = new Date(isoString).getTime();
  if (Number.isNaN(ts)) return null;
  return Math.round((Date.now() - ts) / 60000);
}

function isOlderThan(isoString, maxAgeMs) {
  if (!isoString) return true;
  const t = new Date(isoString).getTime();
  if (Number.isNaN(t)) return true;
  return Date.now() - t > maxAgeMs;
}

function snapshotIdForLane(deskLane) {
  if (deskLane === 'voices') return 2;
  if (deskLane === 'watchdogs') return 3;
  if (deskLane === 'defense_ops') return 4;
  if (deskLane === 'indicators') return 5;
  if (deskLane === 'statements') return 6;
  return 1;
}

/**
 * @param {string | undefined} deskLane
 */
function normalizeDeskLane(deskLane) {
  if (
    deskLane === 'voices' ||
    deskLane === 'watchdogs' ||
    deskLane === 'defense_ops' ||
    deskLane === 'indicators' ||
    deskLane === 'statements' ||
    deskLane === 'osint'
  ) {
    return deskLane;
  }
  return 'osint';
}

function createDeskBuildLaneSourceIdLoader() {
  const cache = new Map();
  return async (deskLane) => {
    if (!cache.has(deskLane)) {
      cache.set(deskLane, fetchSourceIdsForDeskLane(deskLane));
    }
    return cache.get(deskLane);
  };
}

function deskLabelForLane(deskLane) {
  if (deskLane === 'voices') return 'Voices';
  if (deskLane === 'watchdogs') return 'Watchdogs';
  if (deskLane === 'defense_ops') return 'Defense';
  if (deskLane === 'indicators') return 'Indicators';
  if (deskLane === 'statements') return 'Statements';
  return 'OSINT';
}

/** @param freshness {{ latestFetchedAt: string | null, latestSuccessfulIngestAt: string | null }} */
function buildFreshnessMeta(freshness, deskState, deskLane, newestLeadPublishedAt = null) {
  const thresholdMinutes = parseStaleAfterMinutes();
  const latestFetchedAgeMinutes = freshness ? ageMinutesFromIso(freshness.latestFetchedAt) : null;
  const latestSuccessAgeMinutes = freshness
    ? ageMinutesFromIso(freshness.latestSuccessfulIngestAt)
    : null;
  const newestLeadAgeMinutes = newestLeadPublishedAt ? ageMinutesFromIso(newestLeadPublishedAt) : null;

  let staleReason = null;
  if (deskState === 'snapshot') {
    const label = deskLabelForLane(deskLane);
    staleReason = `Live database read failed; showing the last saved ${label} desk. Nothing weaker was substituted.`;
  } else if (deskState === 'stale') {
    staleReason = `Latest item fetch and last successful ingest are both older than ${thresholdMinutes} minutes (override with INTEL_DESK_STALE_AFTER_MINUTES). Ingest may be stalled.`;
  }

  return {
    thresholdMinutes,
    latestFetchedAgeMinutes,
    latestSuccessAgeMinutes,
    newestLeadPublishedAt,
    newestLeadAgeMinutes,
    staleReason,
    deskState,
  };
}

/**
 * Builds display-layer freshness metadata from what is actually rendered to users.
 * This is separate from ingest freshness — it tracks how old the visible items are,
 * not just how recently the pipeline ran.
 */
function buildDisplayFreshness({ leadItems, secondaryLeadItems, items }) {
  const visibleItems = [
    ...(Array.isArray(leadItems) ? leadItems : []),
    ...(Array.isArray(secondaryLeadItems) ? secondaryLeadItems : []),
    ...(Array.isArray(items) ? items : []),
  ];

  const validPublished = visibleItems
    .map((it) => it?.publishedAt)
    .filter(Boolean)
    .map((iso) => ({ iso, t: new Date(iso).getTime() }))
    .filter((x) => Number.isFinite(x.t))
    .sort((a, b) => b.t - a.t);

  const newestVisiblePublishedAt = validPublished[0]?.iso ?? null;
  const leadPublishedAt =
    Array.isArray(leadItems) && leadItems[0]?.publishedAt ? leadItems[0].publishedAt : null;

  function ageHours(iso) {
    if (!iso) return null;
    const t = new Date(iso).getTime();
    if (!Number.isFinite(t)) return null;
    return (Date.now() - t) / 3600000;
  }

  const newestVisibleAgeHours = ageHours(newestVisiblePublishedAt);
  const leadAgeHours = ageHours(leadPublishedAt);

  let displayFreshnessState = 'empty';
  if (newestVisibleAgeHours !== null) {
    if (newestVisibleAgeHours <= 12) displayFreshnessState = 'fresh';
    else if (newestVisibleAgeHours <= 48) displayFreshnessState = 'recent';
    else if (newestVisibleAgeHours <= 96) displayFreshnessState = 'aging';
    else displayFreshnessState = 'stale';
  }

  return {
    newestVisiblePublishedAt,
    leadPublishedAt,
    newestVisibleAgeHours,
    leadAgeHours,
    displayFreshnessState,
    hasStaleDisplay: displayFreshnessState === 'stale' || displayFreshnessState === 'empty',
  };
}

function parseRelevanceExplanations(raw) {
  if (!Array.isArray(raw)) return [];
  const out = [];
  for (const e of raw) {
    if (!e || typeof e !== 'object') continue;
    const ruleId = e.ruleId;
    const message = e.message;
    if (typeof ruleId === 'string' && typeof message === 'string') {
      const meta =
        e.meta && typeof e.meta === 'object' && !Array.isArray(e.meta) ? e.meta : undefined;
      out.push(meta ? { ruleId, message, meta } : { ruleId, message });
    }
  }
  return out;
}

function mapRowsToDeskItems(rows) {
  return rows
    .filter((r) => r.sources)
    .map((r) => {
      const s = r.sources;
      const provenanceClass = s.provenance_class;
      const clusterKeys =
        r.cluster_keys && typeof r.cluster_keys === 'object' && !Array.isArray(r.cluster_keys)
          ? r.cluster_keys
          : {};
      const structured =
        r.structured && typeof r.structured === 'object' && !Array.isArray(r.structured)
          ? r.structured
          : {};
      const missionTags = Array.isArray(r.mission_tags) ? r.mission_tags : [];
      const surfaceState =
        r.surface_state === 'downranked' || r.surface_state === 'suppressed'
          ? r.surface_state
          : 'surfaced';
      const relevanceScore =
        typeof r.relevance_score === 'number' && Number.isFinite(r.relevance_score)
          ? r.relevance_score
          : 50;

      const trust = computeTrustWarnings({
        source: {
          trustWarningMode: s.trust_warning_mode || 'none',
          trustWarningLevel: s.trust_warning_level || 'info',
          requiresIndependentVerification: Boolean(s.requires_independent_verification),
          heroEligibilityMode: s.hero_eligibility_mode || 'normal',
          trustWarningText: typeof s.trust_warning_text === 'string' ? s.trust_warning_text : null,
        },
        item: {
          title: r.title,
          summary: r.summary,
          sourceSlug: s.slug,
          institutionalArea: typeof r.institutional_area === 'string' ? r.institutional_area : 'unknown',
          missionTags,
          clusterKeys,
        },
      });

      const relevanceExplanations = [
        ...parseRelevanceExplanations(r.relevance_explanations),
        ...(Array.isArray(trust.trustRuleExplanations) ? trust.trustRuleExplanations : []),
      ];

      let imageUrl = r.image_url ?? null;
      if (isFederalRegisterCanonical(r.canonical_url)) {
        imageUrl = null;
      }
      if (shouldSkipFeedImageCandidate(imageUrl)) {
        imageUrl = null;
      } else if (imageUrl) {
        imageUrl = polishFeedCardImageUrl(imageUrl) ?? imageUrl;
      }

      return {
        id: r.id,
        title: r.title,
        summary: r.summary,
        canonicalUrl: r.canonical_url,
        imageUrl,
        publishedAt: r.published_at,
        fetchedAt: r.fetched_at,
        provenanceClass,
        sourceName: s.name,
        sourceSlug: s.slug,
        trustWarningMode: s.trust_warning_mode || 'none',
        trustWarningLevel: s.trust_warning_level || 'info',
        requiresIndependentVerification: Boolean(s.requires_independent_verification),
        heroEligibilityMode: s.hero_eligibility_mode || 'normal',
        trustWarningText: typeof s.trust_warning_text === 'string' ? s.trust_warning_text : null,
        stateChangeType: r.state_change_type,
        deskLane: (typeof s.desk_lane === 'string' ? s.desk_lane : null) ?? r.desk_lane,
        contentUseMode: r.content_use_mode,
        sourceFamily: typeof s.source_family === 'string' ? s.source_family : 'general',
        indicator_class: r.indicator_class ?? null,
        structured,
        clusterKeys,
        relevanceScore,
        surfaceState,
        suppressionReason: r.suppression_reason ?? null,
        missionTags,
        branchOfGovernment: typeof r.branch_of_government === 'string' ? r.branch_of_government : 'unknown',
        institutionalArea: typeof r.institutional_area === 'string' ? r.institutional_area : 'unknown',
        relevanceExplanations,
        isDuplicateLoser: false,
        whyItMatters: whyItMattersStub(provenanceClass, r.state_change_type, clusterKeys),
        ...trust,
      };
    });
}

function partitionDuplicateLosers(withDup) {
  const main = [];
  const duplicates = [];
  for (const it of withDup) {
    if (it.isDuplicateLoser) duplicates.push(it);
    else main.push(it);
  }
  return { main, duplicates };
}

function sortByPublishedDesc(a, b) {
  const ta = a.publishedAt ? new Date(a.publishedAt).getTime() : 0;
  const tb = b.publishedAt ? new Date(b.publishedAt).getTime() : 0;
  return tb - ta;
}

function compareForLead(a, b) {
  const da = typeof a.displayPriority === 'number' ? a.displayPriority : 0;
  const db = typeof b.displayPriority === 'number' ? b.displayPriority : 0;
  if (db !== da) return db - da;
  return compareDeskItems(a, b);
}

async function addDisplayFields(items, deskLane) {
  const lane = deskLane ?? 'osint';
  const { mode, byId } = await prefetchThemeAttentionByItemId(
    (Array.isArray(items) ? items : []).map((it) => ({
      id: it.id,
      sourceSystem: 'intel',
      sourceSlug: it.sourceSlug,
      canonicalUrl: it.canonicalUrl,
    })),
  );
  return items.map((it) => {
    const r = computeDisplayPriority({
      title: it.title,
      summary: it.summary,
      provenanceClass: it.provenanceClass,
      sourceSlug: it.sourceSlug,
      stateChangeType: it.stateChangeType,
      missionTags: it.missionTags,
      branchOfGovernment: it.branchOfGovernment,
      institutionalArea: it.institutionalArea,
      relevanceScore: it.relevanceScore,
      clusterKeys: it.clusterKeys,
      publishedAt: it.publishedAt,
      trustWarningMode: it.trustWarningMode,
      ceremonialOrLowSubstance: Boolean(it.ceremonial_or_low_substance),
      deskLane: lane,
      contentUseMode: it.contentUseMode,
      sourceFamily: it.sourceFamily,
      structured: it.structured,
      surfaceState: it.surfaceState,
      isDuplicateLoser: Boolean(it.isDuplicateLoser),
      themeAttention: byId.get(it.id) ?? null,
      themeRankingMode: mode,
    });
    const promotionDecision = {
      lane,
      surfaceState: it.surfaceState,
      provenanceClass: it.provenanceClass,
      sourceFamily: it.sourceFamily ?? null,
      stateChangeType: it.stateChangeType,
      relevanceScore: it.relevanceScore,
      displayPriority: r.displayPriority,
      displayBucket: r.displayBucket,
      freshness: {
        publishedAt: it.publishedAt ?? null,
        fetchedAt: it.fetchedAt ?? null,
      },
      trust: {
        trustWarningMode: it.trustWarningMode ?? 'none',
        trustWarningLevel: it.trustWarningLevel ?? 'info',
        requiresIndependentVerification: Boolean(it.requiresIndependentVerification),
        official_claim: Boolean(it.official_claim),
        contested_claim: Boolean(it.contested_claim),
        ceremonial_or_low_substance: Boolean(it.ceremonial_or_low_substance),
      },
      explain: {
        // Keep these arrays bounded for payload size while preserving debuggability.
        display: Array.isArray(r.displayExplanations) ? r.displayExplanations.slice(0, 8) : [],
        ingest: Array.isArray(it.relevanceExplanations) ? it.relevanceExplanations.slice(0, 10) : [],
        themeAttention: r.themeAttentionRanking
          ? {
              mode: r.themeAttentionRanking.mode,
              eligible: r.themeAttentionRanking.eligible,
              contribution: r.themeAttentionRanking.contribution,
              appliedContribution: r.themeAttentionRanking.appliedContribution,
              reasons: r.themeAttentionRanking.reasons,
              matchedThemeId: r.themeAttentionRanking.matchedThemeId,
            }
          : null,
      },
    };
    return {
      ...it,
      ...r,
      promotionDecision,
      ...(r.themeAttentionRanking ? { themeAttentionRanking: r.themeAttentionRanking } : {}),
    };
  });
}

function applyCreatorCorroborationBridge(items, creatorSupportItems, deskLane) {
  if (!Array.isArray(items) || items.length === 0) return items;

  const annotateWithoutBoost = (item) => ({
    ...item,
    baseDisplayPriority:
      typeof item.displayPriority === 'number' && Number.isFinite(item.displayPriority)
        ? item.displayPriority
        : 50,
    creatorCorroborationBoost: 0,
    creatorCorroboration: null,
  });

  if (deskLane === 'voices') return items.map(annotateWithoutBoost);
  if (!Array.isArray(creatorSupportItems) || creatorSupportItems.length === 0) {
    return items.map(annotateWithoutBoost);
  }

  const decisions = computeCreatorCorroborationBridge(items, creatorSupportItems, { maxBoost: 4 });
  if (decisions.length === 0) return items.map(annotateWithoutBoost);

  const decisionById = new Map();
  for (const decision of decisions) {
    const existing = decisionById.get(decision.targetItemId);
    if (!existing || decision.boost > existing.boost) {
      decisionById.set(decision.targetItemId, decision);
    }
  }

  return items.map((item) => {
    const decision = decisionById.get(item.id);
    const baseDisplayPriority =
      typeof item.displayPriority === 'number' && Number.isFinite(item.displayPriority)
        ? item.displayPriority
        : 50;
    const missionScope = assessMissionScope({
      title: item.title,
      summary: item.summary,
    });
    const missionEligible = missionScope.scopeState !== 'off_topic';

    // Final guardrail for live desk composition: creator corroboration is a bounded nudge for an
    // already-eligible desk item, not an independent surfacing path for weak or off-topic items.
    if (!decision || !missionEligible) {
      return annotateWithoutBoost(item);
    }

    const themeApplied =
      typeof item.themeAttentionRanking?.appliedContribution === 'number'
        ? item.themeAttentionRanking.appliedContribution
        : 0;
    const capped = capShortWindowCreatorBoost({
      requestedBoost: decision.boost,
      themeAppliedContribution: themeApplied,
    });
    if (capped.boost <= 0) {
      return annotateWithoutBoost(item);
    }

    const displayPriority = Math.min(100, baseDisplayPriority + capped.boost);
    const creatorCorroboration = {
      applied: true,
      boost: capped.boost,
      clusterId: decision.clusterId,
      representativeId: decision.representativeId,
      eventType: decision.eventType,
      reasons: Array.isArray(decision.reasons) ? decision.reasons : [],
      contributions: Array.isArray(decision.contributions) ? decision.contributions : [],
      corroboration: decision.corroboration ?? null,
      creatorConvergence: decision.creatorConvergence ?? null,
      combinedCreatorCapApplied: capped.capped,
    };
    const bridgeExplanation = {
      ruleId: 'live_desk:creator_corroboration_bridge',
      message: capped.capped
        ? `Bounded creator corroboration bridge +${capped.boost} (capped against Theme Memory longitudinal attention).`
        : `Bounded creator corroboration bridge +${capped.boost} from trusted creator convergence around the same live story.`,
      meta: {
        boost: capped.boost,
        clusterId: decision.clusterId,
        representativeId: decision.representativeId,
        creatorReasons: creatorCorroboration.reasons,
        combinedCreatorCapApplied: capped.capped,
      },
    };
    const displayExplanations = Array.isArray(item.displayExplanations)
      ? [
          bridgeExplanation,
          ...item.displayExplanations,
        ]
      : [bridgeExplanation];

    return {
      ...item,
      displayPriority,
      baseDisplayPriority,
      creatorCorroborationBoost: capped.boost,
      creatorCorroboration,
      displayExplanations,
      promotionDecision: item.promotionDecision
        ? {
            ...item.promotionDecision,
            displayPriority,
            creatorCorroboration,
            explain: {
              ...item.promotionDecision.explain,
              display: Array.isArray(displayExplanations) ? displayExplanations.slice(0, 8) : [],
            },
          }
        : item.promotionDecision,
    };
  });
}

/**
 * Annotate each item with freshness eligibility for every display surface,
 * then demote displayBucket when the item exceeds the lead or secondary budget.
 *
 * Demoting displayBucket keeps the explanation trail intact and prevents
 * score-based promotion from overriding an age gate after this step.
 * pickLeadBlock reads freshness.leadEligible directly as the hard gate.
 */
function annotateDeskFreshness(items, deskLane, now = new Date()) {
  const nowMs = now instanceof Date ? now.getTime() : now;
  return items.map((it) => {
    const pub = it.publishedAt ?? null;
    const ageHours = getAgeHours(pub, nowMs);
    const bucket = ageHours !== null ? getFreshnessBucket(ageHours) : null;

    const leadEligible = isFreshEnoughForSurface({ item: pub, lane: deskLane, surface: 'lead', now: nowMs });
    const secondaryEligible = isFreshEnoughForSurface({ item: pub, lane: deskLane, surface: 'secondary', now: nowMs });
    const defaultVisibleEligible = isFreshEnoughForSurface({ item: pub, lane: deskLane, surface: 'defaultVisible', now: nowMs });
    const homepageEligible = isFreshEnoughForSurface({ item: pub, lane: deskLane, surface: 'homepage', now: nowMs });

    const leadExclusionReason = getFreshnessExclusionReason({ item: pub, lane: deskLane, surface: 'lead', now: nowMs });
    const secondaryExclusionReason = getFreshnessExclusionReason({ item: pub, lane: deskLane, surface: 'secondary', now: nowMs });
    const defaultVisibleExclusionReason = getFreshnessExclusionReason({ item: pub, lane: deskLane, surface: 'defaultVisible', now: nowMs });

    let displayBucket = it.displayBucket;
    let displayExplanations = it.displayExplanations;

    if (displayBucket === 'lead' && !leadEligible) {
      displayBucket = 'secondary';
      displayExplanations = [
        {
          ruleId: 'freshness:lead_age_gate',
          message: leadExclusionReason ?? 'Demoted from lead: exceeds lead max-age for this desk.',
        },
        ...(Array.isArray(it.displayExplanations) ? it.displayExplanations : []),
      ];
    } else if (displayBucket === 'secondary' && !secondaryEligible) {
      displayBucket = 'routine';
      displayExplanations = [
        {
          ruleId: 'freshness:secondary_age_gate',
          message: secondaryExclusionReason ?? 'Demoted from secondary: exceeds secondary max-age for this desk.',
        },
        ...(Array.isArray(it.displayExplanations) ? it.displayExplanations : []),
      ];
    }

    return {
      ...it,
      displayBucket,
      displayExplanations,
      freshness: {
        ...(it.freshness ?? {}),
        ageHours,
        bucket,
        leadEligible,
        secondaryEligible,
        defaultVisibleEligible,
        homepageEligible,
        leadExclusionReason,
        secondaryExclusionReason,
        defaultVisibleExclusionReason,
      },
    };
  });
}

function pickLeadBlock(items, opts = {}) {
  const deskLane = opts.deskLane ?? 'osint';
  const maxLead = typeof opts.maxLead === 'number' ? opts.maxLead : 1;
  const maxSecondary = typeof opts.maxSecondary === 'number' ? opts.maxSecondary : 4;
  const maxPerArea = typeof opts.maxPerArea === 'number' ? opts.maxPerArea : 2;
  const maxPerSource =
    typeof opts.maxPerSource === 'number' ? opts.maxPerSource : deskLane === 'watchdogs' ? 1 : 2;

  const surfacedOnly = items.filter((it) => it.surfaceState === 'surfaced' && !it.isDuplicateLoser);
  const surfacedWithTrustNotes = surfacedOnly.map((it) => {
    const mode = it.heroEligibilityMode || 'normal';
    if (mode === 'normal') return it;

    const reasons = [];
    if (mode === 'demote_low_substance') {
      if (it.ceremonial_or_low_substance) reasons.push('low_substance');
      if (!(it.displayBucket === 'lead' || it.displayBucket === 'secondary')) reasons.push('routine_bucket');
    } else if (mode === 'never_hero_without_corroboration') {
      if (it.displayBucket !== 'lead') reasons.push('no_corroboration_signal');
    }

    if (reasons.length === 0) return it;
    const msg =
      mode === 'demote_low_substance'
        ? `Hero-ineligible by source policy (${mode}): ${reasons.join(', ')}.`
        : `Hero-ineligible by source policy (${mode}).`;

    const existing = Array.isArray(it.relevanceExplanations) ? it.relevanceExplanations : [];
    return {
      ...it,
      relevanceExplanations: [...existing, { ruleId: 'trust:hero_ineligible', message: msg }],
    };
  });

  function isTrustEligibleForSlot(it) {
    if (it.sourceSlug === 'indicator-pentagon-pizza') return false;
    const mode = it.heroEligibilityMode || 'normal';
    if (mode === 'normal') return true;
    if (mode === 'demote_low_substance') {
      return !it.ceremonial_or_low_substance && (it.displayBucket === 'lead' || it.displayBucket === 'secondary');
    }
    if (mode === 'never_hero_without_corroboration') {
      // Conservative default: do not hero these sources without stronger deterministic “impact” signals.
      return it.displayBucket === 'lead';
    }
    return true;
  }

  // Lead pool: hard lead freshness gate — every item reaching this point should already
  // be annotated by annotateDeskFreshness; unannotated items are excluded defensively.
  const leadPool = surfacedWithTrustNotes.filter((it) => {
    if (it.freshness?.leadEligible !== true) return false;
    return isTrustEligibleForSlot(it);
  });

  // Secondary pool: relaxed to the secondary age budget — items too old for lead but still
  // within the secondary window are eligible here. Same trust rules apply.
  const secondaryPool = surfacedWithTrustNotes.filter((it) => {
    if (it.freshness?.secondaryEligible !== true) return false;
    return isTrustEligibleForSlot(it);
  });

  const rankedForLead = [...leadPool].sort(compareForLead);
  const rankedForSecondary = [...secondaryPool].sort(compareForLead);

  const picked = [];
  const countsByArea = new Map();
  const countsBySource = new Map();

  function canTake(it) {
    const area = it.institutionalArea || 'unknown';
    const src = it.sourceSlug || 'unknown';
    const ca = countsByArea.get(area) ?? 0;
    const cs = countsBySource.get(src) ?? 0;
    return ca < maxPerArea && cs < maxPerSource;
  }

  function take(it) {
    picked.push(it);
    const area = it.institutionalArea || 'unknown';
    const src = it.sourceSlug || 'unknown';
    countsByArea.set(area, (countsByArea.get(area) ?? 0) + 1);
    countsBySource.set(src, (countsBySource.get(src) ?? 0) + 1);
  }

  // Lead item: must pass lead freshness gate.
  for (const it of rankedForLead) {
    if (!canTake(it)) continue;
    take(it);
    break;
  }

  // Secondary lead items: may be beyond the lead window but must pass the secondary gate.
  for (const it of rankedForSecondary) {
    if (picked.length >= maxLead + maxSecondary) break;
    if (picked.some((x) => x.id === it.id)) continue;
    if (!canTake(it)) continue;
    take(it);
  }

  const pickedIds = new Set(picked.map((p) => p.id));
  const remainder = items
    .filter((it) => !pickedIds.has(it.id))
    .map((it) => surfacedWithTrustNotes.find((x) => x.id === it.id) ?? it);

  return { leadItems: picked.slice(0, maxLead), secondaryLeadItems: picked.slice(maxLead), remainder };
}

/** Bounded OG:image backfill (direct fetch — avoids nested unstable_cache inside desk cache). */
async function enrichDeskItemsWithOg(items, opts = {}) {
  if (!opts?.enabled) return;
  const max = Math.min(6, Math.max(0, Number(opts.max) ?? 2));
  const concurrency = Math.min(2, Math.max(1, Number(opts.concurrency) ?? 1));
  if (!Array.isArray(items) || max === 0) return;

  const need = [];
  for (const it of items) {
    if (!it?.canonicalUrl || it.imageUrl) continue;
    if (isFederalRegisterCanonical(it.canonicalUrl)) continue;
    need.push(it);
    if (need.length >= max) break;
  }
  if (need.length === 0) return;

  const limit = pLimit(concurrency);
  await Promise.all(
    need.map((it) =>
      limit(async () => {
        try {
          const og = await fetchOgImageFetchCached(it.canonicalUrl);
          if (og && !shouldSkipFeedImageCandidate(og)) it.imageUrl = polishFeedCardImageUrl(og) ?? og;
        } catch {
          // best-effort only; do not block the desk response
        }
      }),
    ),
  );
}

async function enrichLeadDeskImagesWithOg(leadItems, secondaryLeadItems) {
  const leads = Array.isArray(leadItems) ? leadItems : [];
  const secondary = Array.isArray(secondaryLeadItems) ? secondaryLeadItems : [];
  await enrichDeskItemsWithOg([...leads, ...secondary], {
    enabled: true,
    max: Math.min(2, leads.length + secondary.length),
    concurrency: 1,
  });
}

/** OG thumbnails for accountability overlay rows still missing art (bounded; same helper as main list). */
async function enrichAccountabilityHighlightsWithOg(highlights) {
  if (!parseOgFallbackEnabled()) return;
  const rows = Array.isArray(highlights) ? highlights : [];
  if (rows.length === 0) return;
  const limit = pLimit(1);
  await Promise.all(
    rows.map((h) =>
      limit(async () => {
        if (!h?.canonicalUrl || h.imageUrl) return;
        try {
          const og = await fetchOgImageFetchCached(h.canonicalUrl);
          if (og && !shouldSkipFeedImageCandidate(og)) h.imageUrl = polishFeedCardImageUrl(og) ?? og;
        } catch {
          // best-effort only
        }
      }),
    ),
  );
}

/**
 * @param {string} deskLane
 */
async function buildLiveIntelDesk(deskLane, opts = {}) {
  const lane = normalizeDeskLane(deskLane);
  const snapId = snapshotIdForLane(lane);
  const ogFallbackEnabled = parseOgFallbackEnabled();
  const useSnapshotFallback = opts.useSnapshotFallback !== false;
  const getLaneSourceIds = createDeskBuildLaneSourceIdLoader();

  if (!intelDbConfigured()) {
    return {
      configured: false,
      stale: false,
      dataStale: false,
      snapshotFallback: false,
      liveReadOk: false,
      items: [],
      preCapCandidates: [],
      leadItems: [],
      secondaryLeadItems: [],
      suppressedItems: [],
      duplicateItems: [],
      metadataOnlyItems: [],
      storyClusters: { counts: { total: 0, multiItem: 0, singleton: 0 }, items: [] },
      message: 'Supabase credentials not configured.',
      freshness: null,
      freshnessMeta: null,
      deskLane: lane,
    };
  }

  let freshness = { latestFetchedAt: null, latestSuccessfulIngestAt: null };
  try {
    const laneSourceIds = await getLaneSourceIds(lane);
    freshness = await fetchIntelFreshnessForDeskLane(lane, { laneSourceIds });
  } catch (e) {
    console.warn('[liveIntel] freshness query failed:', e);
  }

  const staleAfterMs = parseStaleAfterMinutes() * 60 * 1000;
  const limSurfaced =
    typeof opts.surfacedFetchLimit === 'number' && Number.isFinite(opts.surfacedFetchLimit)
      ? opts.surfacedFetchLimit
      : parseDeskSurfacedFetchLimit();
  const limDown =
    typeof opts.downrankedFetchLimit === 'number' && Number.isFinite(opts.downrankedFetchLimit)
      ? opts.downrankedFetchLimit
      : parseDeskDownrankedFetchLimit();
  const limSupp =
    typeof opts.suppressedFetchLimit === 'number' && Number.isFinite(opts.suppressedFetchLimit)
      ? opts.suppressedFetchLimit
      : parseDeskSuppressedFetchLimit();
  const maxVisible =
    typeof opts.maxVisibleItems === 'number' && Number.isFinite(opts.maxVisibleItems)
      ? opts.maxVisibleItems
      : parseDeskMaxVisibleItems();

  let surfacedRows;
  let downRows;
  let supRows;
  try {
    const laneSourceIds = await getLaneSourceIds(lane);
    ;[surfacedRows, downRows, supRows] = await Promise.all([
      fetchSurfacedSourceItemsForLive(limSurfaced, lane, { laneSourceIds }),
      fetchDownrankedSourceItemsForLive(limDown, lane, { laneSourceIds }),
      limSupp > 0
        ? fetchSuppressedSourceItemsForLive(limSupp, lane, { laneSourceIds })
        : Promise.resolve([]),
    ]);
  } catch (e) {
    const msg = e instanceof Error ? e.message : String(e);
    if (useSnapshotFallback) {
      try {
        const snap = await loadLiveDeskSnapshot(snapId);
        if (snap?.items?.length) {
          const mergedFresh = snap.freshness ?? freshness;
          const freshnessMeta = {
            ...buildFreshnessMeta(mergedFresh, 'snapshot', lane),
            display: buildDisplayFreshness({
              leadItems: snap.leadItems ?? [],
              secondaryLeadItems: snap.secondaryLeadItems ?? [],
              items: snap.items ?? [],
            }),
          };
          return {
            configured: true,
            stale: true,
            dataStale: true,
            snapshotFallback: true,
            liveReadOk: false,
            items: snap.items,
            preCapCandidates: snap.preCapCandidates ?? snap.items ?? [],
            leadItems: snap.leadItems ?? [],
            secondaryLeadItems: snap.secondaryLeadItems ?? [],
            suppressedItems: snap.suppressedItems ?? [],
            duplicateItems: snap.duplicateItems ?? [],
            metadataOnlyItems: snap.metadataOnlyItems ?? [],
            storyClusters:
              snap.storyClusters ??
              assembleStoryClusters({
                orderedItems: snap.preCapCandidates ?? snap.items ?? [],
                visibleItems: snap.items ?? [],
                duplicateItems: snap.duplicateItems ?? [],
              }),
            accountabilityHighlights: snap.accountabilityHighlights ?? [],
            freshness: mergedFresh,
            freshnessMeta,
            message: `${deskLabelForLane(lane)} read failed; showing last saved desk (${augmentLiveDeskErrorMessage(msg)})`,
            intelSchemaMisconfigured: isIntelSchemaNotExposedError(msg),
            deskLane: lane,
          };
        }
      } catch (loadErr) {
        console.warn('[liveIntel] snapshot load failed:', loadErr);
      }
    }

    const deskState = 'stale';
    const freshnessMeta = {
      ...buildFreshnessMeta(freshness, deskState, lane),
      display: buildDisplayFreshness({ leadItems: [], secondaryLeadItems: [], items: [] }),
    };
    return {
      configured: true,
      stale: true,
      dataStale: true,
      snapshotFallback: false,
      liveReadOk: false,
      intelSchemaMisconfigured: isIntelSchemaNotExposedError(msg),
      items: [],
      preCapCandidates: [],
      leadItems: [],
      secondaryLeadItems: [],
      suppressedItems: [],
      duplicateItems: [],
      metadataOnlyItems: [],
      storyClusters: { counts: { total: 0, multiItem: 0, singleton: 0 }, items: [] },
      freshness,
      freshnessMeta,
      message: augmentLiveDeskErrorMessage(msg),
      deskLane: lane,
    };
  }

  const { surfacedMain, downMain, metadataOnlyRows } = partitionDeskRowsForPipeline(
    lane,
    surfacedRows,
    downRows,
  );

  const mergedForDesk = [...mapRowsToDeskItems(surfacedMain), ...mapRowsToDeskItems(downMain)];
  const withDup = applyDuplicateClusterOverlay(mergedForDesk);
  const { main, duplicates } = partitionDuplicateLosers(withDup);
  let withDisplay = await addDisplayFields(main, lane);
  withDisplay = applyWatchdogLeadCorroborationRules(withDisplay, lane);
  withDisplay = applyAnecdotalIndicatorClassCaps(withDisplay);

  let creatorSupportItems = [];
  if (lane !== 'voices') {
    try {
      const voicesSourceIds = await getLaneSourceIds('voices');
      const creatorSupportRows = await fetchSurfacedSourceItemsForLive(
        Math.min(limSurfaced, 12),
        'voices',
        { laneSourceIds: voicesSourceIds },
      );
      creatorSupportItems = mapRowsToDeskItems(creatorSupportRows);
    } catch (e) {
      console.warn('[liveIntel] creator support fetch failed:', e);
    }
  }
  withDisplay = applyCreatorCorroborationBridge(withDisplay, creatorSupportItems, lane);
  withDisplay = annotateDeskFreshness(withDisplay, lane);

  let accountabilityHighlights =
    lane === 'voices' || lane === 'indicators'
      ? []
      : computeAccountabilityHighlights(withDisplay, { max: 6 });

  const maxPerSource = lane === 'watchdogs' ? 1 : 2;

  const { leadItems, secondaryLeadItems, remainder } = pickLeadBlock(withDisplay, {
    deskLane: lane,
    maxLead: 1,
    maxSecondary: 4,
    maxPerArea: 2,
    maxPerSource,
  });

  if (ogFallbackEnabled) {
    await enrichLeadDeskImagesWithOg(leadItems, secondaryLeadItems);
  }

  // The remaining list should feel more intentional than provenance-first alone,
  // but remain stable and explainable.
  remainder.sort((a, b) => {
    const ba = a.displayBucket === 'lead' ? 0 : a.displayBucket === 'secondary' ? 1 : 2;
    const bb = b.displayBucket === 'lead' ? 0 : b.displayBucket === 'secondary' ? 1 : 2;
    if (ba !== bb) return ba - bb;
    return compareDeskItems(a, b);
  });

  const preCapCandidates = [...leadItems, ...secondaryLeadItems, ...remainder];
  // Hard visible-age gate: items that failed the defaultVisible freshness annotation are
  // excluded regardless of score. preCapCandidates retains all items for debugging.
  const visible = preCapCandidates
    .filter((it) => it.freshness?.defaultVisibleEligible !== false)
    .slice(0, maxVisible);

  await enrichDeskItemsWithOg(visible, {
    enabled: ogFallbackEnabled,
    max: 2,
    concurrency: 1,
  });

  const imageByDeskItemId = new Map(visible.map((it) => [it.id, it.imageUrl ?? null]));
  accountabilityHighlights = accountabilityHighlights.map((h) => ({
    ...h,
    imageUrl: imageByDeskItemId.get(h.id) ?? h.imageUrl ?? null,
  }));
  await enrichAccountabilityHighlightsWithOg(accountabilityHighlights);

  const suppressedItems = mapRowsToDeskItems(supRows).sort(sortByPublishedDesc);
  const duplicateItems = [...duplicates].sort(sortByPublishedDesc);
  const metadataOnlyItems = mapRowsToDeskItems(metadataOnlyRows).sort(sortByPublishedDesc);
  const storyClusters = assembleStoryClusters({
    orderedItems: preCapCandidates,
    visibleItems: visible,
    duplicateItems,
  });

  const hasEverIngested = Boolean(
    freshness.latestFetchedAt || freshness.latestSuccessfulIngestAt,
  );
  const dataStale =
    hasEverIngested &&
    isOlderThan(freshness.latestFetchedAt, staleAfterMs) &&
    isOlderThan(freshness.latestSuccessfulIngestAt, staleAfterMs);

  const deskState = dataStale ? 'stale' : 'fresh';
  const newestLeadPublishedAt =
    [...leadItems, ...secondaryLeadItems]
      .map((it) => it.publishedAt)
      .filter(Boolean)
      .sort()
      .reverse()[0] ?? null;
  const freshnessMeta = {
    ...buildFreshnessMeta(freshness, deskState, lane, newestLeadPublishedAt),
    display: buildDisplayFreshness({ leadItems, secondaryLeadItems, items: visible }),
  };

  if (opts.saveSnapshot !== false) {
    try {
      await saveLiveDeskSnapshot(snapId, {
        items: visible,
        preCapCandidates,
        suppressedItems,
        duplicateItems,
        leadItems,
        secondaryLeadItems,
        metadataOnlyItems,
        storyClusters,
        accountabilityHighlights,
        freshness,
        freshnessMeta,
      });
    } catch (e) {
      console.warn('[liveIntel] snapshot save failed:', e);
    }
  }

  return {
    configured: true,
    stale: dataStale,
    dataStale,
    snapshotFallback: false,
    liveReadOk: true,
    items: visible,
    preCapCandidates,
    leadItems,
    secondaryLeadItems,
    suppressedItems,
    duplicateItems,
    metadataOnlyItems,
    storyClusters,
    accountabilityHighlights,
    freshness,
    freshnessMeta,
    message: dataStale ? freshnessMeta.staleReason : null,
    deskLane: lane,
  };
}

const getLiveIntelDeskOsint = unstable_cache(
  async () => buildLiveIntelDesk('osint'),
  ['intel-live-desk-v13', 'osint'],
  { revalidate: LIVE_INTEL_DESK_CACHE_TTL_SECONDS, tags: ['intel-live'] },
);

const getLiveIntelDeskVoices = unstable_cache(
  async () => buildLiveIntelDesk('voices'),
  ['intel-live-desk-v13', 'voices'],
  { revalidate: LIVE_INTEL_DESK_CACHE_TTL_SECONDS, tags: ['intel-live'] },
);

const getLiveIntelDeskWatchdogs = unstable_cache(
  async () => buildLiveIntelDesk('watchdogs'),
  ['intel-live-desk-v13', 'watchdogs'],
  { revalidate: LIVE_INTEL_DESK_CACHE_TTL_SECONDS, tags: ['intel-live'] },
);

const getLiveIntelDeskDefenseOps = unstable_cache(
  async () => buildLiveIntelDesk('defense_ops'),
  ['intel-live-desk-v13', 'defense_ops'],
  { revalidate: LIVE_INTEL_DESK_CACHE_TTL_SECONDS, tags: ['intel-live'] },
);

const getLiveIntelDeskIndicators = unstable_cache(
  async () => buildLiveIntelDesk('indicators'),
  ['intel-live-desk-v13', 'indicators'],
  { revalidate: LIVE_INTEL_DESK_CACHE_TTL_SECONDS, tags: ['intel-live'] },
);

const getLiveIntelDeskStatements = unstable_cache(
  async () => buildLiveIntelDesk('statements'),
  ['intel-live-desk-v13', 'statements'],
  { revalidate: LIVE_INTEL_DESK_CACHE_TTL_SECONDS, tags: ['intel-live'] },
);

export async function getLiveIntelDesk(deskLane = 'osint') {
  if (deskLane === 'voices') return getLiveIntelDeskVoices();
  if (deskLane === 'watchdogs') return getLiveIntelDeskWatchdogs();
  if (deskLane === 'defense_ops') return getLiveIntelDeskDefenseOps();
  if (deskLane === 'indicators') return getLiveIntelDeskIndicators();
  if (deskLane === 'statements') return getLiveIntelDeskStatements();
  return getLiveIntelDeskOsint();
}

export async function getLiveIntelDeskUncached(deskLane = 'osint', opts = {}) {
  return buildLiveIntelDesk(deskLane, opts);
}

function toPublicCreatorCorroboration(creatorCorroboration) {
  if (!creatorCorroboration || typeof creatorCorroboration !== 'object') return null;
  return {
    applied: Boolean(creatorCorroboration.applied),
    boost:
      typeof creatorCorroboration.boost === 'number' && Number.isFinite(creatorCorroboration.boost)
        ? creatorCorroboration.boost
        : 0,
    clusterId: creatorCorroboration.clusterId ?? null,
    representativeId: creatorCorroboration.representativeId ?? null,
    eventType: creatorCorroboration.eventType ?? null,
    reasons: Array.isArray(creatorCorroboration.reasons)
      ? creatorCorroboration.reasons
          .filter((reason) => typeof reason === 'string' && reason.trim())
          .slice(0, 6)
          .map((reason) => truncatePublicText(reason, PUBLIC_DESK_STRING_LIMITS.creatorReason))
      : [],
  };
}

function truncatePublicText(value, maxLength) {
  if (typeof value !== 'string') return null;
  if (value.length <= maxLength) return value;
  return `${value.slice(0, maxLength - 1).trimEnd()}…`;
}

function toPublicExplanation(explanation) {
  if (!explanation || typeof explanation !== 'object') return null;
  if (typeof explanation.ruleId !== 'string' || typeof explanation.message !== 'string') return null;
  return {
    ruleId: explanation.ruleId,
    message: truncatePublicText(explanation.message, PUBLIC_DESK_STRING_LIMITS.explanationMessage),
  };
}

function toPublicDeskItem(item) {
  if (!item || typeof item !== 'object') return item;
  return {
    id: item.id,
    title: truncatePublicText(item.title, PUBLIC_DESK_STRING_LIMITS.title),
    summary: truncatePublicText(item.summary, PUBLIC_DESK_STRING_LIMITS.summary),
    canonicalUrl: item.canonicalUrl ?? null,
    imageUrl: item.imageUrl ?? null,
    publishedAt: item.publishedAt ?? null,
    provenanceClass: item.provenanceClass ?? null,
    sourceName: item.sourceName ?? null,
    sourceSlug: item.sourceSlug ?? null,
    trustBadges: Array.isArray(item.trustBadges) ? item.trustBadges.slice(0, 2) : [],
    trustExplain: truncatePublicText(item.trustExplain, PUBLIC_DESK_STRING_LIMITS.trustExplain),
    trustWarningText: truncatePublicText(
      item.trustWarningText,
      PUBLIC_DESK_STRING_LIMITS.trustWarningText,
    ),
    contentUseMode: item.contentUseMode ?? null,
    deskLane: item.deskLane ?? null,
    displayBucket: item.displayBucket ?? null,
    displayPriority:
      typeof item.displayPriority === 'number' && Number.isFinite(item.displayPriority)
        ? item.displayPriority
        : null,
    surfaceState: item.surfaceState ?? 'surfaced',
    suppressionReason: item.suppressionReason ?? null,
    missionTags: Array.isArray(item.missionTags) ? item.missionTags.slice(0, 4) : [],
    whyItMatters: truncatePublicText(item.whyItMatters, PUBLIC_DESK_STRING_LIMITS.whyItMatters),
    relevanceExplanations: Array.isArray(item.relevanceExplanations)
      ? item.relevanceExplanations.map(toPublicExplanation).filter(Boolean).slice(0, 6)
      : [],
    displayExplanations: Array.isArray(item.displayExplanations)
      ? item.displayExplanations.map(toPublicExplanation).filter(Boolean).slice(0, 6)
      : [],
    clusterKeys:
      item.clusterKeys && typeof item.clusterKeys === 'object' && !Array.isArray(item.clusterKeys)
        ? item.clusterKeys
        : {},
    stateChangeType: item.stateChangeType ?? null,
    trustWarningMode: item.trustWarningMode ?? 'none',
    institutionalArea: item.institutionalArea ?? 'unknown',
    creatorCorroboration: toPublicCreatorCorroboration(item.creatorCorroboration),
    isDuplicateLoser: Boolean(item.isDuplicateLoser),
  };
}

function toPublicAccountabilityHighlight(item) {
  if (!item || typeof item !== 'object') return item;
  return {
    id: item.id,
    title: truncatePublicText(item.title, PUBLIC_DESK_STRING_LIMITS.title),
    canonicalUrl: item.canonicalUrl ?? null,
    imageUrl: item.imageUrl ?? null,
    provenanceClass: item.provenanceClass ?? null,
    sourceName: item.sourceName ?? null,
    eventClass: item.eventClass ?? null,
    explanations: Array.isArray(item.explanations)
      ? item.explanations
          .map(toPublicExplanation)
          .filter(Boolean)
          .slice(0, 4)
          .map((explanation) => ({
            ...explanation,
            message: truncatePublicText(
              explanation.message,
              PUBLIC_DESK_STRING_LIMITS.accountabilityExplanation,
            ),
          }))
      : [],
  };
}

function buildPublicStoryClustersForDesk(desk) {
  const leadItems = Array.isArray(desk?.leadItems) ? desk.leadItems.map(toPublicDeskItem) : [];
  const secondaryLeadItems = Array.isArray(desk?.secondaryLeadItems)
    ? desk.secondaryLeadItems.map(toPublicDeskItem)
    : [];
  const visibleItems = Array.isArray(desk?.items) ? desk.items.map(toPublicDeskItem) : [];
  const duplicateItems = Array.isArray(desk?.duplicateItems)
    ? desk.duplicateItems.map(toPublicDeskItem)
    : [];

  return assembleStoryClusters({
    orderedItems: [...leadItems, ...secondaryLeadItems, ...visibleItems],
    visibleItems,
    duplicateItems,
  });
}

export function buildPublicDeskPayload(desk) {
  const leadItems = Array.isArray(desk?.leadItems) ? desk.leadItems.map(toPublicDeskItem) : [];
  const secondaryLeadItems = Array.isArray(desk?.secondaryLeadItems)
    ? desk.secondaryLeadItems.map(toPublicDeskItem)
    : [];
  const items = Array.isArray(desk?.items) ? desk.items.map(toPublicDeskItem) : [];
  const suppressedItems = Array.isArray(desk?.suppressedItems)
    ? desk.suppressedItems.map(toPublicDeskItem)
    : [];
  const duplicateItems = Array.isArray(desk?.duplicateItems)
    ? desk.duplicateItems.map(toPublicDeskItem)
    : [];
  const metadataOnlyItems = Array.isArray(desk?.metadataOnlyItems)
    ? desk.metadataOnlyItems.map(toPublicDeskItem)
    : [];

  return {
    configured: Boolean(desk?.configured),
    stale: Boolean(desk?.stale),
    dataStale: Boolean(desk?.dataStale),
    snapshotFallback: Boolean(desk?.snapshotFallback),
    liveReadOk: Boolean(desk?.liveReadOk),
    intelSchemaMisconfigured: Boolean(desk?.intelSchemaMisconfigured),
    items,
    leadItems,
    secondaryLeadItems,
    suppressedItems,
    duplicateItems,
    metadataOnlyItems,
    storyClusters: buildPublicStoryClustersForDesk({
      leadItems,
      secondaryLeadItems,
      items,
      duplicateItems,
    }),
    accountabilityHighlights: Array.isArray(desk?.accountabilityHighlights)
      ? desk.accountabilityHighlights.map(toPublicAccountabilityHighlight)
      : [],
    freshness: desk?.freshness ?? null,
    freshnessMeta: desk?.freshnessMeta ?? null,
    message: desk?.message ?? null,
    deskLane: desk?.deskLane ?? 'osint',
  };
}

export function measureSerializedPublicDeskPayloadBytes(payload) {
  return Buffer.byteLength(JSON.stringify(payload), 'utf8');
}

export function describePublicDeskPayloadBudget(payload, budgetBytes = PUBLIC_OSINT_PAYLOAD_BUDGET_BYTES) {
  const payloadBytes = measureSerializedPublicDeskPayloadBytes(payload);
  return {
    payloadBytes,
    budgetBytes,
    remainingBytes: budgetBytes - payloadBytes,
    withinBudget: payloadBytes < budgetBytes,
  };
}

function sortedClusterEntries(clusterKeys) {
  if (!clusterKeys || typeof clusterKeys !== 'object' || Array.isArray(clusterKeys)) return [];
  return Object.entries(clusterKeys)
    .filter((entry) => typeof entry[0] === 'string' && typeof entry[1] === 'string' && entry[1])
    .sort((a, b) => {
      const ka = `${a[0]}:${a[1]}`;
      const kb = `${b[0]}:${b[1]}`;
      return ka.localeCompare(kb);
    });
}

function buildDuplicateDecisionIndex(items) {
  const groups = new Map();
  for (const item of items) {
    for (const [key, value] of sortedClusterEntries(item.clusterKeys)) {
      const compositeKey = `${key}:${value}`;
      const arr = groups.get(compositeKey) ?? [];
      arr.push(item);
      groups.set(compositeKey, arr);
    }
  }

  const decisions = new Map();
  for (const [compositeKey, group] of groups) {
    if (group.length < 2) continue;
    const ranked = [...group].sort((a, b) => {
      const pa = provenanceRank(a.provenanceClass);
      const pb = provenanceRank(b.provenanceClass);
      if (pa !== pb) return pa - pb;
      if (b.relevanceScore !== a.relevanceScore) return b.relevanceScore - a.relevanceScore;
      const ta = a.publishedAt ? new Date(a.publishedAt).getTime() : 0;
      const tb = b.publishedAt ? new Date(b.publishedAt).getTime() : 0;
      if (ta !== tb) return tb - ta;
      return a.id.localeCompare(b.id);
    });

    const winner = ranked[0];
    for (const current of ranked) {
      const existing = decisions.get(current.id);
      if (!existing || compositeKey < existing.duplicateClusterKey) {
        decisions.set(current.id, {
          duplicateClusterKey: compositeKey,
          duplicateSelection: current.id === winner.id ? 'winner' : 'loser',
          duplicateWinnerId: winner.id,
        });
      }
    }
  }

  return decisions;
}

function buildMissionScopeDebug(item) {
  const mission = assessMissionScope({
    title: item.title,
    summary: item.summary,
  });
  return {
    reason: mission.reason,
    positiveHits: mission.positiveHits,
    sportsHits: mission.sportsHits,
    softOffTopicHits: mission.softOffTopicHits,
  };
}

function buildStoryClustersForDesk(desk) {
  return assembleStoryClusters({
    orderedItems: Array.isArray(desk?.preCapCandidates) ? desk.preCapCandidates : [],
    visibleItems: Array.isArray(desk?.items) ? desk.items : [],
    duplicateItems: Array.isArray(desk?.duplicateItems) ? desk.duplicateItems : [],
  });
}

function buildDebugItem(item, listName, duplicateDecisionIndex) {
  const duplicateDecision = duplicateDecisionIndex.get(item.id) ?? null;
  const nonSurfaceReasons = [];
  if (item.surfaceState === 'suppressed') nonSurfaceReasons.push('suppressed');
  if (item.surfaceState === 'downranked') nonSurfaceReasons.push('downranked');
  if (item.isDuplicateLoser) nonSurfaceReasons.push('duplicate_cluster_loser');
  const fr = item.freshness;
  if (fr) {
    if (fr.leadEligible === false) nonSurfaceReasons.push('excluded_from_lead_by_freshness');
    if (fr.secondaryEligible === false) nonSurfaceReasons.push('excluded_from_secondary_by_freshness');
    if (fr.defaultVisibleEligible === false) nonSurfaceReasons.push('excluded_by_default_visible_age_gate');
    if (
      fr.ageHours === null &&
      (fr.leadEligible === false || fr.secondaryEligible === false || fr.defaultVisibleEligible === false)
    ) {
      nonSurfaceReasons.push('excluded_missing_or_invalid_published_at');
    }
  }
  return {
    id: item.id,
    listName,
    nonSurfaceReasons,
    title: item.title,
    url: item.canonicalUrl,
    sourceSlug: item.sourceSlug,
    deskLane: item.deskLane ?? null,
    provenanceClass: item.provenanceClass,
    publishedAt: item.publishedAt,
    relevance_score: item.relevanceScore,
    surface_state: item.surfaceState,
    suppression_reason: item.suppressionReason,
    baseDisplayPriority:
      typeof item.baseDisplayPriority === 'number' ? item.baseDisplayPriority : null,
    displayPriority: typeof item.displayPriority === 'number' ? item.displayPriority : null,
    creatorCorroborationBoost:
      typeof item.creatorCorroborationBoost === 'number' ? item.creatorCorroborationBoost : 0,
    displayBucket: item.displayBucket ?? null,
    missionScope: buildMissionScopeDebug(item),
    creatorCorroboration: item.creatorCorroboration
      ? {
          applied: Boolean(item.creatorCorroboration.applied),
          boost:
            typeof item.creatorCorroboration.boost === 'number'
              ? item.creatorCorroboration.boost
              : 0,
          clusterId: item.creatorCorroboration.clusterId ?? null,
          representativeId: item.creatorCorroboration.representativeId ?? null,
          eventType: item.creatorCorroboration.eventType ?? null,
          reasons: Array.isArray(item.creatorCorroboration.reasons)
            ? item.creatorCorroboration.reasons
            : [],
          contributions: Array.isArray(item.creatorCorroboration.contributions)
            ? item.creatorCorroboration.contributions
            : [],
          corroboration: item.creatorCorroboration.corroboration ?? null,
          creatorConvergence: item.creatorCorroboration.creatorConvergence ?? null,
        }
      : null,
    themeAttentionRanking: item.themeAttentionRanking
      ? {
          mode: item.themeAttentionRanking.mode ?? null,
          eligible: Boolean(item.themeAttentionRanking.eligible),
          contribution:
            typeof item.themeAttentionRanking.contribution === 'number'
              ? item.themeAttentionRanking.contribution
              : 0,
          appliedContribution:
            typeof item.themeAttentionRanking.appliedContribution === 'number'
              ? item.themeAttentionRanking.appliedContribution
              : 0,
          reasons: Array.isArray(item.themeAttentionRanking.reasons)
            ? item.themeAttentionRanking.reasons
            : [],
          ineligibleReasons: Array.isArray(item.themeAttentionRanking.ineligibleReasons)
            ? item.themeAttentionRanking.ineligibleReasons
            : [],
          matchedThemeId: item.themeAttentionRanking.matchedThemeId ?? null,
          attentionIsNotCorroboration: true,
          debug: item.themeAttentionRanking.debug ?? null,
        }
      : null,
    duplicateClusterKey: duplicateDecision?.duplicateClusterKey ?? null,
    duplicateSelection: duplicateDecision?.duplicateSelection ?? 'unique',
    duplicateWinnerId: duplicateDecision?.duplicateWinnerId ?? null,
    freshness: item.freshness
      ? {
          ageHours: typeof item.freshness.ageHours === 'number' ? item.freshness.ageHours : null,
          bucket: item.freshness.bucket ?? null,
          leadEligible: item.freshness.leadEligible ?? null,
          secondaryEligible: item.freshness.secondaryEligible ?? null,
          defaultVisibleEligible: item.freshness.defaultVisibleEligible ?? null,
          homepageEligible: item.freshness.homepageEligible ?? null,
          leadExclusionReason: item.freshness.leadExclusionReason ?? null,
          secondaryExclusionReason: item.freshness.secondaryExclusionReason ?? null,
          defaultVisibleExclusionReason: item.freshness.defaultVisibleExclusionReason ?? null,
        }
      : null,
    shortExplanations: {
      relevance: Array.isArray(item.relevanceExplanations) ? item.relevanceExplanations.slice(0, 10) : [],
      display: Array.isArray(item.displayExplanations) ? item.displayExplanations.slice(0, 8) : [],
    },
  };
}

export async function getLiveIntelDeskDebug(deskLane = 'osint') {
  const lane = normalizeDeskLane(deskLane);
  const desk = await getLiveIntelDesk(lane);
  const preCapCandidates = Array.isArray(desk.preCapCandidates) ? desk.preCapCandidates : [];
  const duplicatePool = [...preCapCandidates, ...(desk.duplicateItems ?? [])];
  const duplicateDecisionIndex = buildDuplicateDecisionIndex(duplicatePool);
  const visibleIds = new Set(Array.isArray(desk.items) ? desk.items.map((item) => item.id) : []);

  return {
    deskLane: lane,
    generatedAt: new Date().toISOString(),
    configured: desk.configured,
    stale: desk.stale,
    dataStale: desk.dataStale,
    snapshotFallback: desk.snapshotFallback,
    liveReadOk: desk.liveReadOk,
    message: desk.message ?? null,
    freshness: desk.freshness ?? null,
    freshnessMeta: desk.freshnessMeta ?? null,
    counts: {
      visible: Array.isArray(desk.items) ? desk.items.length : 0,
      lead: Array.isArray(desk.leadItems) ? desk.leadItems.length : 0,
      secondaryLead: Array.isArray(desk.secondaryLeadItems) ? desk.secondaryLeadItems.length : 0,
      preCapCandidates: preCapCandidates.length,
      suppressed: Array.isArray(desk.suppressedItems) ? desk.suppressedItems.length : 0,
      duplicates: Array.isArray(desk.duplicateItems) ? desk.duplicateItems.length : 0,
      metadataOnly: Array.isArray(desk.metadataOnlyItems) ? desk.metadataOnlyItems.length : 0,
    },
    leadItemIds: Array.isArray(desk.leadItems) ? desk.leadItems.map((item) => item.id) : [],
    secondaryLeadItemIds: Array.isArray(desk.secondaryLeadItems)
      ? desk.secondaryLeadItems.map((item) => item.id)
      : [],
    storyClusters: desk.storyClusters ?? buildStoryClustersForDesk(desk),
    items: {
      preCapCandidates: preCapCandidates.map((item, index) => {
        const base = buildDebugItem(item, 'pre_cap_candidate', duplicateDecisionIndex);
        const madeVisible = visibleIds.has(item.id);
        const cappedReason = madeVisible ? [] : ['capped'];
        return {
          ...base,
          preCapRank: index + 1,
          madeVisible,
          nonSurfaceReasons: [...new Set([...(base.nonSurfaceReasons ?? []), ...cappedReason])],
        };
      }),
      visible: Array.isArray(desk.items)
        ? desk.items.map((item) => buildDebugItem(item, 'visible', duplicateDecisionIndex))
        : [],
      suppressed: Array.isArray(desk.suppressedItems)
        ? desk.suppressedItems.map((item) => buildDebugItem(item, 'suppressed', duplicateDecisionIndex))
        : [],
      duplicates: Array.isArray(desk.duplicateItems)
        ? desk.duplicateItems.map((item) => buildDebugItem(item, 'duplicates', duplicateDecisionIndex))
        : [],
      metadataOnly: Array.isArray(desk.metadataOnlyItems)
        ? desk.metadataOnlyItems.map((item) => buildDebugItem(item, 'metadata_only', duplicateDecisionIndex))
        : [],
    },
  };
}

const getPublicLiveIntelDeskOsint = unstable_cache(
  async () => buildPublicDeskPayload(await buildLiveIntelDesk('osint')),
  ['intel-public-desk-v3', 'osint'],
  { revalidate: LIVE_INTEL_DESK_CACHE_TTL_SECONDS, tags: ['intel-live'] },
);

const getPublicLiveIntelDeskStatements = unstable_cache(
  async () => buildPublicDeskPayload(await buildLiveIntelDesk('statements')),
  ['intel-public-desk-v3', 'statements'],
  { revalidate: LIVE_INTEL_DESK_CACHE_TTL_SECONDS, tags: ['intel-live'] },
);

export async function getPublicLiveIntelDesk(deskLane = 'osint') {
  if (deskLane === 'osint') return getPublicLiveIntelDeskOsint();
  if (deskLane === 'statements') return getPublicLiveIntelDeskStatements();
  return getLiveIntelDesk(deskLane);
}

const HOMEPAGE_BRIEFING_DESK_LIMITS = {
  surfacedFetchLimit: 32,
  downrankedFetchLimit: 10,
  suppressedFetchLimit: 0,
  maxVisibleItems: 32,
};

const getHomepageBriefingDeskOsint = unstable_cache(
  async () =>
    buildLiveIntelDesk('osint', {
      ...HOMEPAGE_BRIEFING_DESK_LIMITS,
      useSnapshotFallback: false,
      saveSnapshot: false,
    }),
  ['homepage-briefing-desk-v4', 'osint'],
  { revalidate: LIVE_INTEL_DESK_CACHE_TTL_SECONDS, tags: ['homepage-briefing', 'intel-live'] },
);

const getHomepageBriefingDeskWatchdogs = unstable_cache(
  async () =>
    buildLiveIntelDesk('watchdogs', {
      ...HOMEPAGE_BRIEFING_DESK_LIMITS,
      useSnapshotFallback: false,
      saveSnapshot: false,
    }),
  ['homepage-briefing-desk-v4', 'watchdogs'],
  { revalidate: LIVE_INTEL_DESK_CACHE_TTL_SECONDS, tags: ['homepage-briefing', 'intel-live'] },
);

const getHomepageBriefingDeskDefenseOps = unstable_cache(
  async () =>
    buildLiveIntelDesk('defense_ops', {
      ...HOMEPAGE_BRIEFING_DESK_LIMITS,
      useSnapshotFallback: false,
      saveSnapshot: false,
    }),
  ['homepage-briefing-desk-v4', 'defense_ops'],
  { revalidate: LIVE_INTEL_DESK_CACHE_TTL_SECONDS, tags: ['homepage-briefing', 'intel-live'] },
);

const getHomepageBriefingDeskVoices = unstable_cache(
  async () =>
    buildLiveIntelDesk('voices', {
      ...HOMEPAGE_BRIEFING_DESK_LIMITS,
      useSnapshotFallback: false,
      saveSnapshot: false,
    }),
  ['homepage-briefing-desk-v4', 'voices'],
  { revalidate: LIVE_INTEL_DESK_CACHE_TTL_SECONDS, tags: ['homepage-briefing', 'intel-live'] },
);

export async function getLiveIntelDeskForHomepageBriefing(deskLane = 'osint') {
  if (deskLane === 'voices') return getHomepageBriefingDeskVoices();
  if (deskLane === 'watchdogs') return getHomepageBriefingDeskWatchdogs();
  if (deskLane === 'defense_ops') return getHomepageBriefingDeskDefenseOps();
  return getHomepageBriefingDeskOsint();
}
