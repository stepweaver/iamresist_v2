...
function pickLeadBlock(items, opts = {}) {
  ...
  // Lead pool: hard lead freshness gate.
  const leadPool = surfacedWithTrustNotes.filter((it) => {
    if (it.freshness?.leadEligible === false) return false;
  ...
  const secondaryPool = surfacedWithTrustNotes.filter((it) => {
    if (it.freshness?.secondaryEligible === false) return false;
    return isTrustEligibleForSlot(it);
  });
...
async function buildLiveIntelDesk(deskLane, opts = {}) {
  ...
  const newestLeadPublishedAt =
    [...leadItems, ...secondaryLeadItems]
      ...
      .sort()
      .reverse()[0] ?? null;
  const freshnessMeta = buildFreshnessMeta(freshness, deskState, lane, newestLeadPublishedAt);
