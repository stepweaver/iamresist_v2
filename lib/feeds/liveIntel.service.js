...
import { passesAgeGate } from '@/lib/intel/freshnessPolicy';
...
/**
 * Enforce per-lane max-age budgets as hard display eligibility rules.
 *
 * Items that exceed the lead or secondary age window have their displayBucket
 * downgraded before pickLeadBlock runs, so a stale high-score item can never
 * win a lead slot over a genuinely current item with the same or lower score.
 * Explanations are preserved for debuggability.
 */
function applyDeskFreshnessGates(items, deskLane) {
  return items.map((it) => {
    const pub = it.publishedAt ?? null;
    if (!pub) return it;

    if (it.displayBucket === 'lead' && !passesAgeGate(pub, deskLane, 'lead')) {
      return {
        ...it,
        ...
        displayExplanations: [
          {
            ...
            message: 'Demoted from lead: exceeds lead max-age for this desk.',
          },
    ...
    if (it.displayBucket === 'secondary' && !passesAgeGate(pub, deskLane, 'secondary')) {
      return {
        ...it,
        ...
        displayExplanations: [
          {
            ...
            message: 'Demoted from secondary: exceeds secondary max-age for this desk.',
          },
          ...(Array.isArray(it.displayExplanations) ? it.displayExplanations : []),
        ],
      };
...
async function buildLiveIntelDesk(deskLane, opts = {}) {
  ...
  withDisplay = applyDeskFreshnessGates(withDisplay, lane);
  ...
  // Hard visible-age gate: items older than defaultVisibleMaxAgeHours are excluded from the
  // displayed desk regardless of score. preCapCandidates retains all items for debugging.
  const visible = preCapCandidates
    .filter((it) => passesAgeGate(it.publishedAt, lane, 'defaultVisible'))
    .slice(0, maxVisible);
