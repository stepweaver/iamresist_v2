...
import { getCachedPageBlocks, getPageBlocks } from '@/lib/notion-blocks';
...
const SUPPORTED_BODY_BLOCK_TYPES = [
  'paragraph',
  'bulleted_list_item',
  'numbered_list_item',
  'heading_1',
...
function supportedBlockValue(block) {
  if (!block || !SUPPORTED_BODY_BLOCK_TYPES.includes(block.type)) return null;
  return block[block.type] ?? null;
}

function richTextHref(part) {
  if (!part || typeof part !== 'object') return null;
  const href = typeof part.href === 'string' && part.href.trim() ? part.href.trim() : null;
  if (href) return href;
  const link = part.text?.link?.url;
...
function extractRawUrls(text) {
  ...
  const matches = source.match(RAW_URL_RE) ?? [];
  return matches.map((match) => match.replace(/[),.;!?]+$/g, ''));
}

function collectStableUniqueUrls(urls) {
  const out = [];
  const seen = new Set();
...
export function extractWeeklyBriefBody(blocks) {
  ...
  for (const block of Array.isArray(blocks) ? blocks : []) {
    ...
    for (const part of richText) {
      const href = richTextHref(part);
      if (href) urls.push(href);
    }

    if (text) {
      urls.push(...extractRawUrls(text));
    }
  }

  return {
    bodyText: bodyLines.join('\n\n').trim(),
    bodyUrls: collectStableUniqueUrls(urls),
  };
}
...
export async function getWeeklyBriefPageBlocks(id, options = {}) {
  const normalizedId = normalizeNotionPageId(id);
  if (!normalizedId || !notion) return [];

  const { cached = true, maxPages = 5 } = options;

  try {
    return cached ? await getCachedPageBlocks(normalizedId, maxPages) : await getPageBlocks(normalizedId, maxPages);
  } catch (error) {
    if (process.env.NODE_ENV === 'development') {
      console.error('[weeklyBriefs.repo] getWeeklyBriefPageBlocks error:', error);
    }
    return [];
  }
}

export async function getWeeklyBriefBodyText(id, options = {}) {
  const blocks = await getWeeklyBriefPageBlocks(id, options);
  return extractWeeklyBriefBody(blocks).bodyText;
}

export async function getWeeklyBriefBodyUrls(id, options = {}) {
  const blocks = await getWeeklyBriefPageBlocks(id, options);
  return extractWeeklyBriefBody(blocks).bodyUrls;
}

export async function getWeeklyBriefWithBody(id, options = {}) {
  const brief = await getWeeklyBriefById(id);
  if (!brief) return null;

  try {
    const blocks = await getWeeklyBriefPageBlocks(id, options);
    const { bodyText, bodyUrls } = extractWeeklyBriefBody(blocks);
    return {
      ...brief,
      bodyText,
      bodyUrls,
    ...
    if (process.env.NODE_ENV === 'development') {
      console.error('[weeklyBriefs.repo] getWeeklyBriefWithBody error:', error);
    }
    return {
      ...brief,
      bodyText: '',
