...
/**
 * Prefer a configured variant when the catalog names one.
 * Products without a variant id keep the first enabled variant.
 * @param {Array<{id: string|number, is_enabled?: boolean}>|undefined} variants
 * @param {string|number} [preferredVariantId]
 */
export function selectEnabledVariant(variants, preferredVariantId) {
  const list = Array.isArray(variants) ? variants : [];
  const preferred = preferredVariantId == null ? '' : String(preferredVariantId).trim();
  if (preferred) {
    return list.find((v) => String(v.id) === preferred && v.is_enabled) || null;
  }
  return list.find((v) => v.is_enabled) || null;
}

function missingVariantMessage(productId, preferredVariantId) {
  const preferred = preferredVariantId == null ? '' : String(preferredVariantId).trim();
  if (preferred) {
    return productId
      ? `Configured variant ${preferred} is not enabled for product ${productId}`
      : `Configured variant ${preferred} is not enabled for product`;
  }
  return productId
    ? `No enabled variants for product ${productId}`
    : 'No enabled variants found for product';
}
...
export class PrintifyProvider extends FulfillmentProvider {
   ...
   * @param {Array<{printifyProductId: string, printifyVariantId?: string, quantity: number}>} [options.lineItems] - Cart mode
   * @param {string} [options.printifyVariantId] - Single mode: configured Printify variant id
   */
  async createOrder({ orderId, shippingAddress, productSku, quantity, customerEmail, lineItems, printifyVariantId }) {
    ...
    try {
      ...
      if (hasLineItems) {
        for (const item of lineItems) {
          ...
          const variant = selectEnabledVariant(product.variants, item.printifyVariantId);
          if (!variant) {
            return {
              success: false,
              error: missingVariantMessage(item.printifyProductId, item.printifyVariantId),
        ...
        const variant = selectEnabledVariant(product.variants, printifyVariantId);
        if (!variant) {
          return { success: false, error: missingVariantMessage('', printifyVariantId), externalOrderId: null, status: 'error' };
        }
