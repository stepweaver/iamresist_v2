...
import {
  getAllBooks,
  getBookBySlug as getBookBySlugRepo,
  getCurrentBook as getCurrentBookRepo,
} from "../notion/books.repo";
...
export async function listBooks() {
  return unstable_cache(
    ...
    ["bookclub-list-books"],
    { revalidate: REVALIDATE, tags: ["bookclub"] }
  )();
}
...
/** Book with Status = Reading (for homepage card). */
export async function getCurrentBook() {
  return unstable_cache(
    () => getCurrentBookRepo(),
    ["bookclub-current-book"],
    { revalidate: REVALIDATE, tags: ["bookclub"] }
  )();
}
