...
export function getProductDisplayName(sku) {
  ...
  if (sku.startsWith('BELIEVE-NOTHING-STICKER')) return 'BELIEVE NOTHING Bumper Sticker';
...
export async function createCheckoutSession({ priceId, successUrl, cancelUrl }) {
  ...
  const session = await stripe.checkout.sessions.create({
    ...
    metadata: {
      ...
      ...(productConfig.printifyVariantId
        ? { printify_variant_id: productConfig.printifyVariantId }
        : {}),
