...
async function fetchVoicesItems(voices, perVoiceLimit, tagPrefix = 'voices') {
  const limiter = pLimit(
    tagPrefix === 'voices-archive' ? CONCURRENCY_ARCHIVE : CONCURRENCY_HOME
  );
...
function voiceRecencyBoost(iso) {
...
async function buildHomepageVoicesPool() {
  const voices = await getAllVoicesCached({ limit: 80 });
  const voicesToFetch = voices.slice(0, VOICES_TO_FETCH_HOME);
  if (!voicesToFetch.length) return [];

  const allItems = await fetchVoicesItems(
    voicesToFetch,
    ITEMS_PER_VOICE_HOME,
    'voices-home'
  );
    ...
    .map((item) => {
      ...
      return {
        ...
        homepageMissionScore:
          (missionScope.scoreDelta ?? 0) + voiceRecencyBoost(item.publishedAt),
      };
    })
    .filter((item) => item.missionScope.allowedOnHomepageCommentary);

  scored.sort((a, b) => {
    if (b.homepageMissionScore !== a.homepageMissionScore) {
      return b.homepageMissionScore - a.homepageMissionScore;
    }
    return new Date(b.publishedAt || 0) - new Date(a.publishedAt || 0);
  });

  return scored.slice(0, HOMEPAGE_MAX_VOICE_ITEMS);
}
...
async function buildArchiveVoicesFeed() {
  const allVoices = await getAllVoicesCached({ limit: GLOBAL_VOICES_LIMIT });
  const voices = allVoices.slice(0, GLOBAL_VOICES_LIMIT);
  if (!voices.length) return [];

  const merged = await fetchVoicesItems(
    voices,
    GLOBAL_PER_VOICE_LIMIT,
    'voices-archive'
  );
...
async function buildVoiceArchiveFeedBySlug(slug) {
  ...
  const merged = await fetchVoicesItems([voice], GLOBAL_PER_VOICE_LIMIT, 'voices-archive');
  const deduped = dedupeByYoutubeOrUrl(merged);
...
/** Raw homepage pool (RSS voices only), before per-creator limit — for merging with curated/protest on homepage. */
export async function getHomepageVoicesPool() {
  return unstable_cache(buildHomepageVoicesPool, cacheKey('homepage-pool-v4'), {
...
export async function getVoicesFeed() {
  return unstable_cache(buildArchiveVoicesFeed, cacheKey('archive-interleave-v3'), {
    revalidate: REVALIDATE,
    tags: ['voices-feed'],
  })();
}
