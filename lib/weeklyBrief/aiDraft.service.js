          ...
          text: JSON.stringify(
            {
              ...
              instructions: {
                ...
                goals: [
                  ...
                  'Work the selected candidates into the brief as supporting evidence and reading/listening pointers.',
...
async function requestWeeklyBriefDraft(prompt) {
  ...
  const response = await fetch('https://api.openai.com/v1/responses', {
    ...
    body: JSON.stringify({
      model: openaiEnv.WEEKLY_BRIEF_DRAFT_MODEL,
      input: prompt,
      text: {
        format: {
          type: 'json_schema',
  ...
  return {
    ...
    model: json.model ?? openaiEnv.WEEKLY_BRIEF_DRAFT_MODEL,
  ...
  if (selectedCandidates.length === 0) {
    throw new WeeklyBriefDraftValidationError(
      selection.mode === 'explicit_id'
        ? 'No selected weekly candidates matched the provided selectedCandidateIds.'
        : 'No selected weekly candidates matched the Notion page body URLs.',
      {
