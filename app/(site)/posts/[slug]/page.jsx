...
export async function generateMetadata({ params }) {
  ...
  if (!entry) return { title: 'Post Not Found | I AM [RESIST]' };
...
export default async function PostPage({ params }) {
  ...
  return (
    <main className="min-h-screen">
      <div className="mx-auto w-full max-w-[1600px] px-1 sm:px-2 lg:px-3 pt-6 sm:pt-8">
        <nav className="nav-label mb-6 text-xs text-foreground/60" aria-label="Breadcrumb">
          <Link
            href="/"
          ...
          <span className="mx-2 text-foreground/40">/</span>
          <span className="text-foreground font-bold">{entry.title || 'Entry'}</span>
        </nav>
      </div>
        ...
        {blocks.length > 0 ? (
          ...
          <div className="machine-panel border border-border p-6 sm:p-8">
            <p className="prose-copy text-foreground/60 font-mono text-sm">
              [ NO BODY BLOCKS ] — The entry is published in Notion but has no block content
              yet, or blocks could not be loaded.
            </p>
          </div>
        )}
      </div>
