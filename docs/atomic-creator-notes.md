...
Local audio transcription fallback (publisher transcript still wins; required flag; dry-run still writes only a local transcript cache, never creator-notes rows). Transcription is separate from the text-inference provider:
...
Requires `ffmpeg` and faster-whisper on the host. Cache: `tmp/creator-notes-audio-transcripts/`. The production batch uses the same local transcription fallback; install the timer only after a one-episode run succeeds.
...
| `GROQ_API_KEY` | (not in repo) | Required when Groq is selected. Runtime environment only. Never commit a key. |
...
- Groq audio transcription (text inference may use Groq; transcription stays local in this pass)
- silently falling back from Groq to Ollama
...
`GROQ_API_KEY` must already be in the runtime environment. `--transcribe-audio` still uses local faster-whisper; it does not send audio to Groq.
